...
annual_contribution = clean_input(input("Enter how much you will contribute to your Roth 401(k) each year: $"))
employer_match_percentage = float(input("Enter your employer's Roth 401(k) match percentage per dollar (e.g., 50 for 50%): ")) / 100
...
# Employer's annual match contribution
employer_annual_contribution = annual_contribution * employer_match_percentage
...
# Calculate the growth of Roth IRA and Roth 401(k) separately over time (including contributions)
...
# Calculate total Roth 401(k) balance with contributions and employer match
roth_401k_contributions_values = [roth_401k_balance + (annual_contribution + employer_annual_contribution) * (year - current_age) for year in years]

# Separate account values (sum of Roth IRA and Roth 401(k) growth over time)
total_separate_values = [
    roth_ira + roth_401k for roth_ira, roth_401k in zip(roth_ira_values, roth_401k_values)
]
...
print(f"Employer's annual Roth 401(k) match contribution: ${employer_annual_contribution:,.2f}")
print(f"Your annual Roth 401(k) contribution: ${annual_contribution:,.2f}")

print("\n--- Future Value Calculations ---")
print(f"Future value of Roth IRA at retirement (separate): ${roth_ira_values[-1]:,.2f}")
print(f"Future value of Roth 401(k) at retirement (separate): ${roth_401k_values[-1]:,.2f}")
print(f"Future value of Roth 401(k) and Roth IRA combined (separate accounts): ${total_separate_values[-1]:,.2f}")
print(f"Future value of Roth 401(k) with IRA rollover (combined): ${combined_roth_401k_values_immediate[-1]:,.2f}")
